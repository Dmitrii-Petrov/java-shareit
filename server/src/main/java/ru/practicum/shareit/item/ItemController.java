...
public class ItemController {
    ...
    @GetMapping("/{itemId}")
    public ItemDto getItemById(@PathVariable(required = false) Long itemId, @RequestHeader("X-Sharer-User-Id") Long userId) {
        log.info("поулчен запрос GET /items/id");
        return itemService.getItemDtoByItemId(itemId, userId);
    }

    @PostMapping("/{itemId}/comment")
    public CommentDto addComment(@PathVariable(required = false) Long itemId, @RequestHeader("X-Sharer-User-Id") Long userId, @RequestBody CommentDto commentDto) {
    ...
    @PatchMapping("/{itemId}")
    public Item update(@PathVariable Long itemId, @RequestBody ItemDto itemDto, @RequestHeader("X-Sharer-User-Id") Long userId) {
